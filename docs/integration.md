...
Once the transaction has been created, your endpoint can fetch an access token by calling the [create transaction credentials](https://gateway.vr-payment.de/doc/api/web-service#transaction-service--create-transaction-credentials) API endpoint. The access token is returned and passed to the Android Payment SDK.
...
import de.vrpayment.VRPaymentSdk
import de.vrpayment.event.OnResultEventListener
import de.vrpayment.event.PaymentResult
...
class MyApp : Application() {
    ...
    override fun onCreate() {
        super.onCreate()
        VRPaymentSdk.init(listener = object : OnResultEventListener{
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnCheckout.setOnClickListener {
            VRPaymentSdk.instance?.launch(token, this) ?: run {
                Log.e(
                    "Mobile SDK",
