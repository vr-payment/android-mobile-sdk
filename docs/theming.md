...
- `VRPaymentSdk.instance?.setLightTheme(JSONObject)` allows to modify the payment dialog's light theme.
- `VRPaymentSdk.instance?.setDarkTheme(JSONObject)` allows to modify the payment dialog's dark theme.
- `VRPaymentSdk.instance?.setCustomTheme(JSONObject, ThemeEnum)` allows to enforce a specific theme (dark, light or your own).

```kotlin
// ...
import de.vrpayment.enums.ThemeEnum
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        VRPaymentSdk.instance?.setDarkTheme(getCustomTheme())
        VRPaymentSdk.instance?.setLightTheme(getCustomTheme())
    }
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        VRPaymentSdk.instance?.setCustomTheme(getCustomTheme(), ThemeEnum.DARK)
    }
...
```kotlin
  //...
VRPaymentSdk.instance?.setCustomTheme(null, ThemeEnum.DARK)
```
...
Use `setAnimation` method to change the screen change animation. Currently available options are: `AnimationEnum.SLIDE` and `AnimationEnum.BUBBLE`. Default value is `AnimationEnum.SLIDE`. `VRPaymentSdk.instance?.setAnimation(AnimationEnum.BUBBLE)` changes the screen transition animation used inside the payment flow.
